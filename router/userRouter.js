...
userRouter.post("/register", userValidator, async (req, res) => {
    let { email, name, password, number,username,location } = req.body;
    ...
    if (email && password) {
      bcrypt.hash(password, 3, async (err, hash) => {
        ...
        try {
          let user = new UserModel({ email, name, password: hash, number,username,location});
          await user.save();
          res.send({
      ...
      try {
        let user = new UserModel({ name, username, number });
        await user.save();
        res.send({
          message: "User is regsitered",
          status: 1,
